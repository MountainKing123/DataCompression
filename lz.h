...
// LZ compression with chunk-local matching and recent-offsets cache
class LZ
{
public:
    ...
    struct Match
    {
        uint32_t distance = 0;
        uint32_t length = 0;
        [[nodiscard]] bool isValid() const { return length >= 3; }
    };
    ...
    struct CompressOptions
    {
        ...
        size_t maxChainLength   = 32;
        bool lazyMatching       = false;
        uint32_t hashSkipLen    = 32;
        uint32_t hashSkipStride = 4;
    };

    struct EncodedSymbol
    {
        enum Type { Literal, Match } type = Literal;
        uint8_t literal = 0;
        uint32_t distance = 0;
        uint32_t length = 0;
    ...
    // Recent-offset cache operations (also used by LZHuffman for decode)
...
private:
    ...
    static inline uint32_t hash4(const uint8_t* const data);
